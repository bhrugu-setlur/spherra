...
Status: **I4 technical specification approved by Codex and Claude Opus; Spherra naming amendment applied on 2026-08-04**
...
If this document conflicts with the R7 architecture, R7 wins. A material architectural change requires a new design revision and renewed Codex/Claude approval. Benchmark-selected constants may change when the R7 acceptance harness provides evidence, but the evidence and decision must be recorded before code or durable formats are changed.
...
- A living root `AGENTS.md` project guide.
- Tracked Graphify graph, manifest, report, labels, and query-memory outputs plus local Graphify runtime/cache files.
...
The root `AGENTS.md` is the living project guide. Every project change must leave it accurate about completed work, remaining work, and the next executable step. Agents must not claim milestone completion from code presence alone; they must run and record the milestone exit gate.

The Graphify graph is also required project state. After code, test, specification, benchmark-evidence, or status changes:

1. Update `AGENTS.md` when status, constraints, decisions, paths, commands, or next work changed.
2. Run the Graphify incremental-update workflow from the repository root.
3. Verify no semantic refresh is pending and query the changed concepts.
4. Do not report completion while `AGENTS.md` or the graph is stale.

## 11. Approval record

- **Codex:** `APPROVE` for the I4 technical specification.
- **Claude Opus:** explicitly returned `APPROVE` for the complete I4 technical specification.
